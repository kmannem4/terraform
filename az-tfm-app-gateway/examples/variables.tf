...
variable "global_configuration" {
  description = "Global configuration for Application Gateway"
  type = object({
    request_buffering_enabled  = optional(bool)
    response_buffering_enabled = optional(bool)
  })
  default = {}
}
...
variable "ssl_profiles" {
  description = "SSL profiles to allow the backend with Azure Application Gateway"
  type = list(object({
    name                                 = string
...
variable "subnets" {
  description = "subnets for Application Gateway"
  default     = []
}
