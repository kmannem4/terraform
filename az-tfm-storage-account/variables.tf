...
variable "resource_group_name" {
  ...
  default     = "co-wus2-amnoneshared-rg-t02"
  type        = string
}
...
variable "skuname" {
  ...
  validation {
    condition     = try(contains(["Standard_LRS", "Standard_GRS", "Standard_RAGRS", "Standard_ZRS", "Standard_RAGZRS"], var.skuname), true)
    error_message = "The `sku_name` value must be valid. Possible values include Standard_GRS, Standard_RAGRS, Standard_ZRS, Standard_RAGZRS."
  }
}

variable "access_tier" {
  description = "Defines the access tier for Hot and Cool accounts. Valid options are Hot and Cool."
  default     = "Hot"
  type        = string

  validation {
    condition     = try(contains(["Hot", "Cool"], var.access_tier), true)
    error_message = "The `access tier` value must be valid. Possible values include Hot ,Cool."
  }
}
